...
export const AnalyzeMarksView = ({ onSuccess, gradingFormula }) => {
  ...
  const [subjects, setSubjects] = useState([
    { id: '1', subjectName: 'Data Structures & Algorithms', marksObtained: 88, totalMarks: 100, credits: 4 },
    { id: '2', subjectName: 'Database Management Systems', marksObtained: 92, totalMarks: 100, credits: 4 },
    { id: '3', subjectName: 'Discrete Mathematics', marksObtained: 75, totalMarks: 100, credits: 3 },
    { id: '4', subjectName: 'Computer Networks', marksObtained: 82, totalMarks: 100, credits: 3 },
  ]);
  ...
  const handleCalculate = async (e) => {
    ...
    for (let i = 0; i < subjects.length; i++) {
      ...
      if (sub.marksObtained === '' || sub.marksObtained < 0) {
        setError(`Marks obtained for "${sub.subjectName}" cannot be empty or negative.`);
        return;
      }
      if (sub.totalMarks && Number(sub.marksObtained) > Number(sub.totalMarks)) {
        setError(`Marks obtained (${sub.marksObtained}) cannot exceed total marks (${sub.totalMarks}) for "${sub.subjectName}".`);
        return;
      }
    }
    ...
    try {
      const response = await fetch('/api/calculate-marks', {
        method: 'POST',
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 pb-12">
      ...
      <div className="text-center space-y-3">
        <div className="inline-flex items-center justify-center w-11 h-11 rounded-2xl bg-[#7C9DC5]/15 text-[#7C9DC5]">
          <Calculator className="w-5 h-5" />
        </div>
        <h1 className="font-serif-title text-3xl sm:text-4xl font-normal text-[#2D2A26] dark:text-[#FAF7F2]">
          Calculate Performance Using Marks
        </h1>
        <p className="text-[#6E685F] dark:text-[#BDB6AC] text-xs sm:text-sm max-w-xl mx-auto">
          Enter subject names, obtained marks, total marks, and credits. The backend calculates your CGPA, percentage, and overall grade.
        </p>
      </div>
        ...
        <div className="bg-white dark:bg-[#282521] rounded-[20px] p-6 sm:p-8 border border-[#EAE4DC] dark:border-[#3B3630] shadow-xs space-y-6">
          <div className="flex items-center justify-between pb-4 border-b border-[#EAE4DC] dark:border-[#3B3630]">
            <div className="flex items-center gap-2">
              ...
              <h3 className="font-serif-title text-xl font-normal text-[#2D2A26] dark:text-[#FAF7F2]">Subject Entry List</h3>
            </div>

            <button
              type="button"
              id="add-marks-subject-btn"
            ...
            {subjects.map((sub, idx) => (
              ...
              >
                ...
                <div className="col-span-4 sm:col-span-3">
                  <label className="block text-[10px] font-semibold text-[#6E685F] mb-1 sm:hidden">Obtained Marks</label>
                  <input
                    type="number"
                    required
                    min="0"
                    ...
                    onChange={(e) => handleSubjectChange(sub.id, 'marksObtained', e.target.value === '' ? '' : Number(e.target.value))}
                    placeholder="Marks"
                    className="w-full px-3 py-1.5 bg-white dark:bg-[#282521] border border-[#EAE4DC] dark:border-[#3B3630] rounded-lg text-[#2D2A26] dark:text-[#FAF7F2] text-xs font-medium focus:outline-none focus:ring-1 focus:ring-[#8AAE92]"
                  />
                </div>
                ...
                <div className="col-span-4 sm:col-span-2">
                  <label className="block text-[10px] font-semibold text-[#6E685F] mb-1 sm:hidden">Total</label>
                  <input
                    type="number"
                    required
                    min="1"
